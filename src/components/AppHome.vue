  ...
  methods: {
    fetch() {
      ...
      fetchActiveSessions(this.token)
        ...
        .catch((error) => {
          this.error = error;
          Vue.$toast.error(`${error.response.data.message}`, { timeout: 5000 });
          this.$router.push({ name: 'login' });
        });
    },
    fetchPastSessions() {
      fetchInactiveSessions(this.token).then((sessions) => {
    ...
    startSession() {
      addSession(this.token, this.selectedUsers).then((session) => {
