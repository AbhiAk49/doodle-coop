  ...
  <v-hover v-slot="{ hover }">
    ...
    >
      ...
      <v-list-item three-line>
        <v-list-item-content>
          <div class="text-overline mb-4">Created By:</div>
          <v-list-item-title class="text-h5 mb-1">
            {{ session.users[0].name }}
          </v-list-item-title>
          <v-list-item-subtitle class="deep-orange--text text--lighten-2">{{
            session._id
          }}</v-list-item-subtitle>
          <v-list-item-subtitle class="blue-grey--text"
            >@{{ session.time }}</v-list-item-subtitle
          >
          <v-list-item-subtitle class="blue-grey--text text--lighten-1">{{
            formatDate(session.date).formatDate
          }}</v-list-item-subtitle>
